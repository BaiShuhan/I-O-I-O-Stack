...
#include <sys/types.h>
#include <sys/stat.h>
#define __USE_GNU  //“#关键词”是预编译指令。预处理过程扫描源代码，对其进行初步的转换，产生新的源代码（源代码主体部分，不包括预编译指令）提供给编译器。预处理过程先于编译器对源代码进行处理，防止各预编译指令被多次引用。
#include <fcntl.h>
#include <time.h>
#include <string.h> 
...
//#define DEBUG_SCRIPT                //关于DEBUG版本中调试信息输出的设置

#ifdef DEBUG_SCRIPT                   //47-48：若DEBUG_SCRIPT已定义（即有调试信息输出），则printf代替DEBUG_SCRIPT，保留原参数，仅替换命令以对调试信息进行输出。
#define SCRIPT_PRINT printf
#else                                 //49-50：若未定义（即无调试信息输出），则命令被替换为空，不进行输出。
#define SCRIPT_PRINT(fmt,args...) 
#endif
//如此就不需要在要进行输出时一条条添加输出语句，或是在不需进行输出时一条条删除输出语句，使编译与调试更便捷。
...
/* Globals */
long long kilo64; //file size(KB)
long long reclen; //record length即I/O大小(KB)
...
long long get_relative_utime(long long start); // get relative time
...
{
	printf("%s: %d\n", __func__, state);  //__func__在编译时会替换为语句所在函数的函数名（此为setState）
	if(g_state != ERROR)
	{
		g_state = state;
		if(state == ERROR)
...
*/   //tv_usec最大值为1000000微妙即1秒，记到1000000后，它会向tv_sec进1再从0开始
{
	long sec,usec;
	double time;
	double rate;  //速率
	
	time_t t;  //time_t表时间值的长整数变量
	
	if(T1.tv_usec > T2.tv_usec)  //则已记到1000000并进位
	{
	...
	time = (double)sec + (double)usec/1000000;  //时间以秒为单位
	
	if(db_test_enable == 0)  //以文件形式传输
	{
...
{
	const char *s_ProcStat = "/proc/stat";   //Linux中通过/proc/stat等文件计算CPU利用率
	...
	unsigned long active_tick, idle_tick, wait_tick;
		...
		s_Handle = open(s_ProcStat, O_RDONLY);   //以只读的方式打开/proc/stat文件，函数执行成功时返回1，发生错误时返回-1，并指向打开的文件。
		
		if(s_Handle >= 0) 
		{ 
				s_Check = read(s_Handle, &s_StatBuffer[0], sizeof(s_StatBuffer) - 1);   //把s_Handle指向的文件传送(sizeof(s_StatBuffer)-1)个字节到&s_StatBuffer[0]所指的内存中。
			                                                                            //成功返回读取的字节数，出错返回-1并设置error，如果在调用read之前已到达文件末尾，则这次read返回0。
				s_StatBuffer[s_Check] = '\0';   //在上述字符串后加上结束符

				s_String = strstr(&s_StatBuffer[0], s_CPUName); /* Total CPU entry */  //strstr()函数用于判断后面的字符串是否是前面的字符串的子串。如果是，则返回后者在前者首次出现的地址；否则，返回NULL。
				...
				{ 
					...
					//printf("s_Check=%d\n", s_Check); sscanf()函数返回成功调用的值的个数，如上指令5个%lu中被成功调用的个数
...
void init_genrand64(unsigned long long seed)  //生成64位随机数
{
    mt[0] = seed;
...
void get_path(pid_t j_pid, pid_t j_tid)  //pid_t是进程号的类型定义
{
	sprintf(j_p_name, "/proc/%d/task/%d/sched", j_pid, j_tid);  //以/proc/%d/task/%d/sched格式将字符串写入j_p_name
//	strcat(j_p_path,j_p_name);  //将两个字符串连接
...
int init_file(char* filename, long long size)
{
	...
	lseek(fd, 0, SEEK_END);  //获取文件大小并将读写位置指向文件尾。当调用成功时则返回目前的读写位置，也就是距离文件开头多少个字节。若有错误则返回-1，error会存放错误代码。

	buf = malloc(512*1024);  //动态内存分配，用于申请一块连续的指定大小的内存块区域以void*类型返回分配的内存区域地址，当无法知道内存具体位置的时候，想要绑定真正的内存空间，就需要用到动态的分配内存。
	memset(buf, 0xcafe, 512*1024);  //将内存空间buf的512*1024尺寸大小设置为字符0xcafe
...
int thread_main(void* arg)
{
	...
	if(strncmp(pathname, "/dev", 4) == 0)  //如果pathname为/dev
	{
		block_open = 1;
	}
	...
	if(block_open == 1)
	{
		sprintf(filename, "%s", pathname);  //将pathname写入到filename中
	...
	if(tmp_recnum == NULL || recnum == NULL){
		...
		if(n_overlap_entries == 0){        //没有I/O重叠
			...
			for(i = 0; i < numrecs64; i++) { // shuffling the array将数组改组
				long long tmp;

				big_rand=genrand64_int64();

				big_rand = big_rand%numrecs64;
				tmp = tmp_recnum[i];                       //tmp_recnum[i]与tmp_recnum[big_rand]互换
	...
	{
		ret = unlink(filename);  //删除一个文件的目录项并减少它的链接数，若成功则返回0，否则返回-1，错误原因存于error。
//		if(ret != 0)
//			printf("Unlink %s failed\n", filename);
	...
	{
		maddr=(char *)initfile(fd,filebytes64,1,PROT_READ|PROT_WRITE, real_reclen);  //MMAP下备用
	}

	/* align buffer to page_size */
	page_size = sysconf(_SC_PAGESIZE);  //sysconf()返回选项(变量)的当前值
...
//	printf("T%d start\n", thread_num);
	...
	if(g_access == MODE_WRITE || g_access == MODE_RND_WRITE)
	{
		for(i=0; i<numrecs64; i++)
		{
			...
			if(g_sync == MMAP || g_sync == MMAP_AS || g_sync == MMAP_S)  //MMAP下
		 	{
				if(g_access == MODE_RND_WRITE)
				{
				...
				bcopy((long long*)buf,(long long*)wmaddr,(long long)real_reclen);  //将buf的前real_reclen个字节复制到wmaddr中
				if(g_sync == MMAP_AS)
				{
					msync(wmaddr,(size_t)real_reclen,MS_ASYNC);  //刷新变化函数msync()。实现磁盘文件内容与共享内存区中的内容一致，即同步操作。
				}
...
int sql_cb(void* data, int ncols, char** values, char** headers)  //cb=callback
{
	int i;
...
//sqlite3_exec()编译和执行零个或多个SQL 语句，查询的结果返回给回调函数callback。
//db指数据库句柄；sql指待执行的SQL 语句字符串，以’\0’结尾；cb指回调函数，用来处理查询结果，如果不需要回调（比如做insert或者delete操作时），可以输入NULL。
...
int init_db_for_update(sqlite3* db, char* filename,int start ,int trs)
{
	...
	{
	    length =0;
		length += sprintf(sql+length,"BEGIN;");  //将"BEGIN;"写入sql[length]，返回指向的字符串的长度；
...
//	printf("open db : %s\n", filename);
	...
	rc = sqlite3_open(filename, &db);  //打开文件名为filename的数据库，db为数据库句柄。操作正常则返回SQLITE_OK。
...
int readline(FILE *f, char *buffer, size_t len)
{
   ...
   memset(buffer, 0, len);  //向buffer中写入长度为len的0

   for (i = 0; i < len; i++)
   {   
      int c = fgetc(f);   //fgetc(f)指从文件指针f指向的文件中读取一个字符，读取一个字节后，光标位置后移一个字节。

      if (!feof(f))   //检测流上的文件结束符，如果文件结束，则返回非0值，否则返回0，文件结束符只能被clearerr()清除。
      {               //若文件没结束
         if (c == '\r')
            buffer[i] = 0;
         else if (c == '\n')
...
int get_new_fd(int fd_org)  //fd文件描述符
{
	int i;
	int fd_new = -1;
...
int do_script(struct script_entry* se, struct script_thread_time* st)
{
	...
	//printf("thread[%d] %s\n", se->thread_num, se->cmd);  //cmd=command
	if( strncmp(se->cmd, "open", 4) == 0)  //若命令为open
	{
		int fd_org = atoi(se->args[2]);  //把字符串转换成整型数(ASCII)
		
		//printf("open %s, %s, %d\n", se->args[0], se->args[1], fd_org);  //args[]中存储路径及打开模式
		if(se->args[0][0] == '"')
		{
	...
	else if( strncmp(se->cmd, "fsync", 5) == 0)  //同步内存中所有已修改的文件数据到储存设备。
	{
		int fd_new = get_new_fd(atoi(se->args[0]));
	...
	else if( strncmp(se->cmd, "fdatasync", 9) == 0)  //调用
	{
		int fd_new = get_new_fd(atoi(se->args[0]));
	...
	else if( strncmp(se->cmd, "access", 6) == 0)  //检查是否可以读/写某一已存在的文件。
	{
		strcpy(replay_pathname, &se->args[0][1]);
		replay_pathname[strlen(replay_pathname)-1]='\0';
	...
	else if( strncmp(se->cmd, "stat", 4) == 0)  //获得一个与此命名文件有关的信息（到一个struct stat 类型的buf中） 。
	{
	...
	else if( strncmp(se->cmd, "lstat", 5) == 0)  //类似于 stat，但是当命名文件是一个符号连接时， lstat 获取该符号连接的有关信息，而不是由该符号连接引用文件的信息。
	{
		struct stat stat_buf;
		strcpy(replay_pathname, &se->args[0][1]);
		replay_pathname[strlen(replay_pathname)-1]='\0';
	...
	else if( strncmp(se->cmd, "fstat", 5) == 0)  //获得文件描述符 fd 打开文件的相关信息（到一个struct stat 类型的buf中） 。
	{
		struct stat stat_buf;
		int fd_new = get_new_fd(atoi(se->args[0]));
...
int script_thread_main(void* arg)
{
	...
	for(i = 0; i < thread_info->line_count; i++)
	{
		if(gScriptEntry[i].thread_num == thread_num)
		{
			...
			{
				//printf("sleep %lld\n", time_diff);
				usleep(time_diff-1);  //把调用该函数的线程挂起一段时间，单位是微秒
			}
...
int replay_script(void)
{
	...
	*/
	fp = fopen(script_path, "r");  //以只读方式打开文件，该文件必须存在。文件顺利打开后，指向该流的文件指针就会被返回。如果文件打开失败则返回 NULL，并把错误代码存在error中。
	if(fp == NULL)
	{
	...
	*/
	fseek(fp, 0, SEEK_SET);  //fp指向SEEK_SET，执行成功返回0，执行失败不改变fp执行并返回非0值。
	for(i = 0; i < line_count; i++)
	{
		...
		ptr = strtok(line_buf, " ");  //以" "为界分割字符串line_buf。首次调用时，前面的参数指向要分解的字符串，之后再次调用要把它设成NULL。第i次调用返回被分割后的第i个字符串。
...
int main( int argc, char **argv)
{
	...
	while((cret = getopt(argc,argv,"p:f:r:a:y:t:d:n:j:s:g:i:hqL:k:v:T:R:")) != EOF){  //根据选项字符串"p:f:r:a:y:t:d:n:j:s:g:i:hqL:k:v:T:R:"依次分析main函数命令行参数
		switch(cret){                                                                 //选项字符串里的字母后接着冒号“:”，则表示还有相关的参数，全域变量optarg即会指向各选项各自的额外参数。
			case 'p':
	...
	{
		DIR* dir;
		dir = opendir(pathname);  //打开一个目录，在失败的时候返回一个空的指针。
		if(dir == NULL)
		{	
		...
		mkdir(pathname, S_IRWXU | S_IRWXG | S_IRWXO);  //创建目录
		replay_script();
		goto out;
	}	
	...
	if(print_IOPS==1) fclose(pIOPS_fp); // close IOPS file
